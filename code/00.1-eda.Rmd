...
# Assuming whale identifiers are numbers, extract them
#unique_whales <- unique(str_extract_all(data$comments, "\\d+") %>% unlist())
#print(unique_whales)

# Step 2: Extract all unique identifiers from the `comments` column
# Capture whole numbers, formatted identifiers like #21, crc-21, cr21, id21, nps21, and words
unique_identifiers <- unique(str_extract_all(data$comments, "\\b(?:#?\\w+-?\\d+|\\w+)\\b") %>% unlist())

# View all unique identifiers
print(unique_identifiers)

# Identify rows where `comments` column contains just "2"
rows_with_2 <- data %>% 
...
expanded_data <- data %>%
  mutate(whales = str_split(comments, ",\\s*")) %>%
  unnest(whales) %>%
  mutate(whales = if_else(str_detect(whales, "^\\d+$"), whales, "unknown")) %>%
  group_by(across(-count, -whales)) %>%
  summarize(
    count = case_when(
      whales == "unknown" ~ sum(count) - n_distinct(whales),
      TRUE ~ 1
    ),
    whales = whales
  ) %>%
...
write.csv(expanded_data,"/Users/cmantegna/Documents/GitHub/sicb_dp/output/data_adjusted/sounders_added_rows.csv", row.names = FALSE)
...
## I'm a sucker for a wrapped plot... this viz just helps us see what is in the table
```{r}
