...
## working with all whales and Sounder data
...
data2 <- read.csv("/Users/cmantegna/Documents/GitHub/sicb_dp/data/whale_data_no_sounders.csv")
...
data2 <- data2 %>% select(-all_of(columns_to_drop))

# take care of the empties with predetermined values
data2$time[data2$time == ""] <- "0:00"
data2$count[data2$count == ""] <- 1
data2$count[is.na(data2$count)] <- 1
data2$channel[data2$channel == ""] <- "unknown"
data2$source[data2$source == ""] <- "unknown"
...
head(data2)
#write.csv(data2, "/Users/cmantegna/Documents/GitHub/sicb_dp/output/data_adjusted/NONsounders_cleaned.csv", row.names = FALSE)
...
all_identifiers <- str_extract_all(data2$comments, "\\b(?:#?\\w+-?\\d+|\\w+)\\b") 
