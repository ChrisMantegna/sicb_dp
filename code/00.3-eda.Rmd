...
ggplot(monthly_summary, aes(x = month, y = sightings_count, fill = source)) +
...
ggplot(yearly_summary, aes(x = month, y = sightings_count, fill = source)) +
  geom_bar(stat = "identity") +
  labs(
...
data <- data %>%
  mutate(
    ...
    time_parsed = as.POSIXct(time_clean, format = "%H:%M:%S", tz = "UTC"),
    time_of_day = case_when(
      is.na(time_parsed) ~ "Not recorded",                       # Handle NA times
      time_parsed >= as.POSIXct("00:00:01", format = "%H:%M:%S") & 
      time_parsed <= as.POSIXct("05:59:59", format = "%H:%M:%S") ~ "Early Morning",  # 0:01 to 5:59
